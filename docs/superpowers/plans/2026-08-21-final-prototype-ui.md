...
- Produces: exactly 19 prototype screens, no legacy compatibility routes, and `assertNoRuntimePrototypeArtifacts(rootDir)`.
...
Add registry assertions for the 19 screen route keys and for unique concrete paths. Assert that the former `tasks.archived` registration and `/tasks/archived` route are absent; archived tasks are only the current `/tasks?view=archived` list view.
...
Implement a source scan limited to non-test files under `src/pages`, `src/features`, `src/components`, and `src/hooks`. Remove the `prototype` field from `RouteRegistration`. Register `access-denied` at `/403`, remove `tasks.archived` and `/tasks/archived` entirely, and keep archived tasks only as `/tasks?view=archived` state generated by the current Tasks page.
...
Development-stage purity ruling: this branch is a release candidate built from the locked final prototype, not an already released final version. Maintain only the latest target implementation; remove legacy compatibility routes, aliases, deprecated wrappers, old DTO adapters, fallback data paths and dual implementations. Do not preserve them behind comments or shims. Add structural assertions for the former `/tasks/archived` route and any compatibility artifacts found during the final source scan.
