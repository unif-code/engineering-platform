...
Release Gate 不替代领域 Contract；每个 Gate 的具体判定仍回链 00～11 的规范 owner。未被当前 Release 选入或未通过 Capability Activation Gate 的目标能力必须保持关闭。首发与延后能力的唯一阶段选择见 detail 的 [Capability Activation Matrix](./implementation-roadmap-detail.md#24-capability-activation-matrix)。
