...
- Agent 的 Repository 读凭据绑定 Platform Environment、Attempt、Project 与 TTL，只允许读取 Binding 固定的 Repository/Commit；任务分支写凭据只由 Guest 外的 Source Control Credential Broker（由 GitLab Connector 承担）持有，按 Attempt、Repository、任务分支、目标 Commit/内容 Hash、单次用途与短 TTL 逐次签发使用，绝不注入 Sandbox；Attempt 进入 Fence 或清理时读凭据按 [Sandbox Runtime](./04-sandbox-runtime.md) 的清理 Contract 即刻吊销，Broker 同时拒绝该 Attempt 的后续 Push——Guest 内的凭据以只读为上限，写路径必须与不可信代码物理隔离。
- Agent 产生的有效代码必须先在 Sandbox 内形成可核验 Commit，由 Broker 校验请求与 Attempt、Repository、任务分支及目标 Commit/内容 Hash 的绑定后代理执行 Push；Broker 幂等核验远端 `headSha`、把 Push Effect 记入 `SourceControlEffect` Ledger 并收敛未知结果，远端确认的 Commit SHA、Push Effect、实际 Agent、发起人员与 Binding 引用共同进入交付证据——只有远端确认的事实才是交付事实，写凭据不入 Guest 才守得住受控来源。
