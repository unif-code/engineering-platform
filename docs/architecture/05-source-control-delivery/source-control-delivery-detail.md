...
本文是 GitLab Binding、任务分支 Saga、Integration/Formal MR、冻结的 Git/MR/Artifact Integration Baseline 证据、GitLab 外部事实收敛、Formal Review Assignment、Merge 与多仓语义的唯一规范事实源。Requirement、WorkItem、Gate、Decision、Acceptance 与业务状态由 [Requirement Workflow](../02-requirement-workflow/requirement-workflow-detail.md)唯一拥有；本文消费其稳定 ID 和受保护命令，产生 Binding、Branch、MR、Merge 与 Baseline 事实，不复制其状态机。Acceptance Decision 对 `integrationBaselineId/hash` 的绑定、失效与业务状态影响仍只由 02 owner 定义。
...
平台为每个 WorkItem 保存 Route 所要求的 Commit Checkpoint；Requirement 的冻结 Integration Baseline 由当前必需 WorkItem 的 Repository、任务分支 Commit、Integration MR/结果、Artifact Hash、Attempt 引用与验证证据组成。它生成稳定的 `integrationBaselineId/hash`，不能引用持续移动的分支 HEAD。

任何相关 Git/MR 证据的 `headSha`、Artifact 或 Contract 变化，都必须产生新的 Baseline 证据或 `BaselineChanged` 事件，旧 Baseline 保留为历史事实。本文只发布该变化与当前证据结果；02 owner 决定 Acceptance Decision 对 `integrationBaselineId/hash` 的绑定、是否失效以及任何 Requirement/WorkItem 业务动作。
