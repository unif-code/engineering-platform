...
Object Storage 的目标实现是每环境的 Rook-Ceph RGW，职责仅限 S3-compatible 对象服务；实时 Stateful PVC 一律使用逻辑 StorageClass `stateful-rwo-lowlatency`，其 Provider Mapping、Node 与容量由 [09](./09-infrastructure-operations.md) 拥有。每个 Bucket Class 在每个环境绑定版本化 `StorageBinding`（`storageBindingId + generation`，绑定目标 Endpoint/Bucket、凭据引用、加密与 Object Lock 能力证明），对象元数据记录写入时的 Binding Generation，读取与删除按记录的 Binding 定位：`audit-worm` 与 Backup 类（`postgres-backup`、`nats-backup`、`openbao-recovery`）自 V0.1 起始终绑定 Cluster 外 OSS/S3-compatible Repository——Cluster 外 Backup 与 WORM 审计归档不因 Rook-Ceph 激活迁回集群内；`requirement-attachments` 与 `agent-artifacts` 在 Rook-Ceph 激活前同样绑定该 Cluster 外 Repository 作为受完整 Contract 约束的过渡 Primary（Observability Object 仍按 [09](./09-infrastructure-operations.md) 推迟），激活后以新 Binding Generation 写入环境内 RGW，既有对象按原 Binding 继续可读、可恢复而不自动回迁；业务类切换到 RGW 的前提是该 Class 已建立并验证独立 `BackupBinding`（`backupBindingId + generation`，绑定源 Class 与 StorageBinding Generation、目标 Endpoint/Bucket、凭据与加密/Lock 能力引用，生命周期 `PENDING → ACTIVE ⇄ DEGRADED → RETIRED`（`DEGRADED` 在 Watermark 追平并通过校验后回到 `ACTIVE`），目的端按 External 容量链维护自己的容量账本）：Backup Watermark 基于每 Class 严格单调的 Object Sequence（由存储准入在提交事务中分配，不以时间戳或 Version 字符串排序）及其复制 Frontier 与 Gap Set——Frontier 是已复制连续前缀的最高 Sequence，乱序完成进入 Gap Set、归并后推进 Frontier，Gap 超时未归并即 `DEGRADED`；RGW 新对象据此异步复制到 Cluster 外 Repository，Backup Manifest 按 Class + Generation 维护逐 Object Version（含 Sequence）的连续无洞清单与 SHA-256；复制水位滞后超过 RPO 预算或目的端预占失败时 Binding 进入 `DEGRADED`，该 Class 的新写入预占 Fail Closed 并产生 Operations Incident，直到 Watermark 追平回到 `ACTIVE` 或经受控 Rebind 把写入切回 Cluster 外 Binding；Rebind 或 Generation 替换必须先以 Fencing Token 隔离旧 Generation 的复制器，in-flight 复制结果只按其所属 Generation 入账、不得写入新 Generation 的 Manifest，旧 Generation 完成清账后才 `RETIRED`；目的端 Repository 另保存自描述 `BackupCatalog`（位于固定 well-known Key，绑定 Platform Environment 与 `CloudEnvironmentBinding` Generation，含自身 Schema Version、内容 Hash、平台签名与 Signing Key ID，索引 Class、Binding/Backup Generation、Watermark 与 Manifest；其读取凭据与验签材料属于 [08](./08-security-audit-governance.md) 的离线 Recovery Kit，不与被保护数据同存），PostgreSQL 与 RGW 同时丢失时可仅凭 Cluster 外 Repository 自举发现并恢复（恢复顺序仍以 [08](./08-security-audit-governance.md) 的信任链为先）；复制延迟、RPO/RTO 与 Restore Drill 见[参数附录的组件资源包络](./appendix-parameters.md#组件资源包络)，未达标时该 Class 不得切换、新写入仍留在 Cluster 外 Binding——集群内没有 Cluster 外副本的对象不得成为唯一权威副本。无论处于哪个阶段与拓扑，S3-compatible API、精确 Object Version、独立 Bucket Class、TLS/身份、加密、硬 Quota/Capacity Ledger、Backup/Restore、Retention 与 Fail Closed Contract 都不变。Bucket Class 固定为下表八类，每类使用独立 Policy、Credential、Quota 与 Capacity Ledger：
...
- Class Usage 使用保守的 logical stored bytes 口径，聚合当前/非当前 Version、受 Lock 保护对象、Delete Marker/Index、已上传与已预占 Multipart、未完成 GC 的对象与标准化元数据估算；容量证据按 `StorageBinding` 类型采集：RGW Binding 的 Ledger、RGW Stats、Cluster Raw 与最满 OSD，External Binding 的 Ledger 与 Provider 侧 Bucket 用量/配额，各自保留单位与阈值，任一 Gate 更危险时取更严格结果——挑选更宽松口径或混算 logical/raw bytes 会让容量准入失去意义。
- 每次写入或 Backup 预占按目标 `StorageBinding` 类型执行容量链：RGW Binding 依次验证 Product Quota（适用时）、Class Operating/Admission、物理 RGW Size/Object Guard、Cluster Raw 与最满 OSD Gate，External Binding 依次验证 Product Quota（适用时）、Class Operating/Admission 与 Provider 侧配额/用量 Guard，Provider 用量/配额证据受版本化最大陈旧时间约束（值见[参数附录](./appendix-parameters.md#bucket-class-容量账本参数)），证据 unknown、不可得或超过陈旧上限时拒绝新预占（Fail Closed），既有对象读取与已获预占不受影响；Class 的 30 天预测按“当前全部占用 + p95/p99 预期写入 + Locked/Multipart 上界 − 已由权威 Reconciler 证明可安全释放的对象”计算，未验证 Backup、只有 Delete Marker 的 Version、失败 Reconcile 与未完成 GC 都不得计作可回收——乐观回收会在真实写入时耗尽容量。
